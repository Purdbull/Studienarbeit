...
\begin{itemize}
	...
	\item Arduino Nano zur Ultraschallüberwachung %@moritz: Bereichsüberwachung statt Ultraschallüberwachung
\end{itemize}
...
%@moritz: Motorsteuerung besser als Hauptsteuerung (etws verwirrend, da zuvor immer ESP32 als Hauptsteuerung bezeichnet wurde  und auch oben so bezeichnet wird)
...
%@moritz: nochmal nennen, welcher Arduino gemeint ist 
...
Betrachtet wird eine Fahrt von Abschnitt Fünf zu Abschnitt Vier. Regel Eins wird zuerst angewendet, da sie die generellen und nicht die fahrspezifischen Geschwindigkeiten bestimmt. In diesem Fall ergäben sich die Abshchnitte wie folgt:
...
Wie bereits in \autoref{sec:konzeptAktorik} beschrieben, wird der Schrittmotor mithilfe desTreibers DRV8834 von Pololu angesteuert. Die vom Treiber benötigten Signale \textit{clk, M0/M1, en} und \textit{dir} werden dafür von einem Arduino Nano generiert. Zu diesem Zweck wurde die die Library \textit{HighTrainStepper} erstellt, welche in diesem Abschnitt beschrieben werden soll. \\
...
Auf der \textit{clk}-Leitung sollen \acrshort{pwm}-Signale zwischen $100Hz - 1000Hz$ generiert werden. Da der Arduino Nano für weitere Zwecke eingesetzt wird, Währenddessen muss der Arduino dennoch in der Lage sein, weitere Prozesse parallel auszuführen. Um dies zu gewährleisten, wurde die Generierung der  \acrshort{pwm} in einer \acrshort{isr} ausgelagert, welche kontinuierlich durch einen internen Timer des Arduinos aufgerufen wird. \\

%In den folgenden Abschnitten soll zunächst die Konfiguration der Interrupts beschrieben werden.  


\paragraph*{Konfiguration des Interrupt-Registers}\mbox{}\vspace{.1cm}\\
%todo : einheitliche Paragraphs zaubern!
In \autoref{code:interrupt} ist der Code zur Konfiguartion des Interrupt-Registers aufgeführt. Die Funktionen werden nachfolgend beschrieben. 

\begin{lstlisting}[language=c, style=dhpaperdefault]
void Stepper::initInterrupts() {
	cli();                                //clear local interrupt flag
	TCCR1A = 0;                           // reset entire control register A
	TCCR1B = 0;                           // reset entire control register B
	TCCR1B |= (1 << WGM12);               // Schalte Clear Timer on Compare (CTC) Modus ein
	TCCR1B |=  (1 << CS11);                 // CS11 fuer den 8 Bit Prescaler
	TCNT1  = 0;                           // Initialisiere Zaehler/Zeitgeber Register Wert auf 0
...
\end{lstlisting}
\captionof{lstlisting}{\label{code:interrupt}Konfiguration der Timer/Counter Control Registers }

Die Timer des ATmega328-Chip des Arduino Nanos können über das "Timer/Counter Control Register" \textit{TCCR} konfiguriert werden. Zu Beginn wird das gesamte Register zurückgesetzt. Im Byte \textit{TCCR1B} kann mit den Bits \textit{WGM12} und \textit{WGM13} ein "Waveform Generation Mode" eingestellt werden. In diesem Fall wurde das Bit \textit{WGM12} gesetzt, wodurch der Modus "Clear Timer on Compare Match" \acrshort{ctc} aktiviert wird. 

Dabei wird der aktuelle Zählwert von Timer1 fortlaufend mit dem Register \textit{OCR1A} verglichen. Hat Timer1 den Registerwert erreicht, wird die \acrshort{isr}  \textit{TIMER1\_COMPA\_vect} ausgeführt. Innerhalb der Service Routine soll die \acrshort{pwm} durch Umschalten eines Ausgangspins generiert werden. 
...
\paragraph*{Implementierung der Fahrfunktion}\mbox{}\vspace{.1cm}\\
Der Motor wird außerhalb der Library mit folgender Funktion gesteuert: 
\begin{lstlisting}[language=c, style=dhpaperdefault]
void run (int targetSpeed, int targetAcc)
\end{lstlisting}

Mithilfe der Parameters \textit{targetSpeed} und \textit{targetAcc} können die gewünschte Geschwindigkeit sowie Beschleunigung in \% definiert werden. Bei Aufruf der Funktion werden die Parameter in die definierten Wertebereich transfomiert und in die globalen Klassenvariabeln \textit{targetRegVal} und \textit{acc} geschrieben. Die beiden Variabeln werden von der \acrshort{isr} verwendet. 

\paragraph*{Implementierung der \acrshort{isr}}\mbox{}\vspace{.1cm}\\
Bei Aufruf der \acrshort{isr} \textit{TIMER1\_COMPA\_vect} wird die klasseninterne Funktion \textit{isr} aufgerufen. Diese erzeugt das \acrshort{pwm}-Signal durch Umschalten des CLK-Ausgangs. 
Im nächsten Schritt wird der aktuelle Wert von 0CR1A mit dem von angestrebten Registerwert \textit{targetRegVal} verglichen. Je nach Ergebnis wird durch inkrementieren bzw. dekrementieren von 0CR1A der Motor beschleunigt oder verlangsamt. Die Häufigkeit der Veränderung des Zählwertes entspricht dem Wert der Beschleunigung und wird durch die Variable \textit{Acc} bestimmt. 
...
\paragraph*{Vorkonfiguration verschiedener Fahrszenarien}\mbox{}\vspace{.1cm}\\
...
\begin{table}[h]
	\begin{center}
		\begin{tabular}[h]{l|l|l}
			...
			\hline
			0 & Aktivieren des Motors (Bestromung der Spulen) & -\\
			\hline
			1 & Nothalt & speed: 0\%, acc:100\%; Deaktivieren des Motors\\
			\hline
			2 & Normaler Halt & speed: 0\%, acc:50\%; HalbSchrittmodus; Viertelschrittmodus; Deaktivieren des Motors\\
			\hline
			3 & Einfahrt in Ladestation & speed: 0\%, acc:50\%; HalbSchrittmodus; Viertelschrittmodus; Deaktivieren \\
			\hline
...
\texttt{sectionSpeeds} ist ein Array vom Typ int und dient dazu, jedem Abschnitt einen vordefinierten Wert für die Maximalgeschwindigkeit zu geben. \texttt{driveSpeeds} ist ein Array was für jede individuelle Fahrt angepasst wird. Über eine Variable \texttt{driveArrayMaxIndex} kann die Anzahl der Abschnitte reduziert werden für kürzere Fahrten. \\
