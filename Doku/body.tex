...
\begin{figure}[h]
	\begin{center}
		\includegraphics[width=\textwidth]{pwm.pdf}
		\caption{Benutzeroberfläche zur Hochbahnsteuerung}
		\label{pic:pwm}
	\end{center}
\end{figure}

In der Interruptkonfiguration wurde mit dem Bit \textit{CS11} innerhalb des Registers \textit{TCCR1B} wurde der Prescaler mit dem Nenner 8 aktiviert. Als Minimal- und Maximalwerte des Vergleichregisters \textit{OCR1A} wurden die Werte 5500 und 950 ausgewählt. Damit ergibt sich für das \acrshort{pwm}-Signal:
...
Dies wurde mit einer \acrshort{isr}(Interrupt Service Routine) gelöst. Für das Interrupt Register wurde der Modus \acrshort{ctc} (Clear Timer on Compare Match) aktiviert. Nachfolgend wird die Einstellung des Interrupts Registers beschrieben. In den darauffolgenden Abschnitten folgt die Implementierung der \acrshort{isr} (Interrupt Service Routine) sowie die Implementierung weiterer Fahrmodi.  


aufrecht während der Signalgenerierung noch immer Befehle über . 
Dafür soll die Library als Input die Zielgeschwindigkeit in \% 

- Interrupt Register mit COC-Funktion 
- Implementierung der COC-Funktion 
- Berechnung der Registerwerte/ Prescaler nach der Formel für die Taktrate 


Implementierung der Geschwindigkeitsverläufe
- Implementierung der ISR

Implementierung weiterer Fahrmodi 
- Implementierung von Schrittmodi 
- Implementierung von Fahrmodi und Startup

Implementierung als Motor-Library 

Die Motorsteuerung ist auf einem Arduino Nano ausgelagert. 



Der Hochbahncontroller ESP32 kann der Steuerung Befehle erteilen und erhält die aktuelle Geschwindigkeit. In \autoref{pic:motorsteuerung} ist schematisch der Steuerungsablauf  dargestellt. 
...
for (int i = 0; i <= driveArrayMaxIndex; i++) {
	...
	else {driveSpeeds[i] = sectionSpeeds[(to - i)]; }
}
