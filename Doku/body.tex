...
Wie bereits in \autoref{sec:konzeptAktorik} beschrieben, umfasst die Motorsteuerung den Schrittmotortreiber DRV8834 von Pololu und den Arduino Nano als Controller. 
Die Ansteuerung des Treibers erfolgt über die Signalleitungen \textit{clk, M0/M1, en} und \textit{dir}. In \autoref{tbl:pololuDriver} sind die Signale definiert. 
...
Für die Steuerung wurde die Library \textit{HightTrainStepper} geschrieben. Damit werden an der \textit{clk}-Leitung \acrshort{pwm}-Signalen zwischen $100Hz - 1000Hz$ generiert werden. Eine Herausforderung stellte die Kommunikation bzw. Ausführung anderer Befehle während der Generierung der \acrshort{pwm} dar. Dafür wurde der Takt durch Interrupts realisiert. Auf die Implementierungd der Library wird nachfolgend genauer eingegangen.

\textbf{Konfiguration des Interrupt-Registers}
Für die Taktgenerierung wurde der Timer 0 verwendet. Im Timer/Counter Control Register \textit{TCCR0} wird der Modus \textit{COC}(Clear Timer on Compare Match) aktiviert. Dadurch wird fortlaufend das Zählregister \textit{TCNT0} mit dem Wert im Register \textit{OCR0} verglichen. Bei Erreichen des Registerwertes wird die Interrupt Service Routine (\textit{ISR}) ausgeführt und der Timer zurückgesetzt. Innerhalb der ISR wird das \textit{clk}-Signal getoggled. Der ATmega328-Chip des Arduino Nanos besitzt eine Systemtaktrate von $16 Mhz$. Durch Verwendung von sog. Mitilfe der Bits \textit{CS01/CS00} kann im TCCR0 Register ein Prescaler eingestellt werden. Die Taktrate der generierten \acrshort{pwm} errechnet sich damit über: 
