...
\subsection{Konzept für die Aufgabenverteilung auf der Hochbahn}
...
\subsubsection{ESP32 als Schnittstelle zum Server}
...
\subsubsection{Arduino Nano als Hauptsteuerung}
...
\subsubsection{Arduino Nano zur Ultraschallüberwachung}
...
\subsection{Protokoll der Datenübertragung zwischen Hauptsteuerung und ESP32}
...
\subsubsection{Anfragen- und Befehlsarten}
...
Folgende Tabelle zeigt den Header einer Nachricht:
%---------------TODO-------------
\begin{table}[h]
	\begin{center}
		...
		\end{tabular}
	\end{center}
	\caption{Header des Nachrichtenprotokolls}
\end{table}

\subsubsection{Nachrichtencontext (Body)}
...
\begin{table}[h]
	\begin{center}
		\begin{tabular}{r | l | l}
			...
			\hline
			\texttt{000000} - \texttt{011111} & \texttt{00} & Akkumeldung von 0-100\% in ~ 3\% Schritten \\
			\hline
			\texttt{000001} und \texttt{000010} & \texttt{11} & Befehlsakzeptanz \texttt{01} = ja; \texttt{10} = nein\\  
		\end{tabular}
	\end{center}
	\label{tab:body}
	\caption{Body dess Nachrichtenprotokolls}
\end{table}

\subsubsection{Tail}
Wie in \autoref{tab:body} zu erkennen ist, werden nie die letzten beiden Bits verwendet. Diese
